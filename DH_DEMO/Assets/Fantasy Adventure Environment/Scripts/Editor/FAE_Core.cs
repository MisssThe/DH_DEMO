...
namespace FAE
{
    public class FAE_Core : Editor
    {
        ...
        public const string PACKAGE_VERSION = "20181";
        public static string INSTALLED_VERSION = "1.2.5";
        ...
        {
            get { return EditorPrefs.GetBool(ASSET_ABRV + "_VERSION_MISMATCH", false); }
            set { EditorPrefs.SetBool(ASSET_ABRV + "_VERSION_MISMATCH", value); }
        }
        ...
        public static void CheckVersionMismatch()
        {
            ...
            //Remove characters after 'f' or 'b' suffix
            string[] splitString = unityVersion.Split('f');
            splitString = unityVersion.Split('b');

            unityVersion = splitString[0];

            //Remove dots
            unityVersion = unityVersion.Replace(".", string.Empty);


            if (int.Parse(unityVersion) > int.Parse(PACKAGE_VERSION))
            {
                IsVersionMismatch = true;

                if (EditorUtility.DisplayDialog(ASSET_NAME, "The installed " + ASSET_NAME + " version does not match the editor version\n\nPlease re-download the package to get the version designed for this Unity version", "Download"))
                {
                    OpenStorePage();
                }
            }
        ...
        public static void InstallRegularMaterials()
        {
            ...
            {
                Debug.LogError("[FAE] Package for regular materials could not be found!");
            }
        }
        ...
        public static void InstallSubstance()
        {
            ...
            {
                Debug.LogError("[FAE] Package for Substance materials could not be found!");
            }
        }
#endif
    }
    ...
    public class OpenOnImport : AssetPostprocessor
    {
        static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
        {
            foreach (string str in importedAssets)
            {
                ...
                {
                    //FAE_Core.CheckVersionMismatch();

                    if (FAE_Core.IsVersionMismatch)
    ...
    public class SCPE_Window : EditorWindow
    {
        ...
        public static Texture2D HeaderImg
        {
            get
            {
                if (_HeaderImg == null)
                {
                    _HeaderImg = (Texture2D)AssetDatabase.LoadAssetAtPath(SessionState.GetString("HEADERIMG_PATH", ""), typeof(Texture2D));
                }
                return _HeaderImg;
            }
        }
        ...
        {
            EditorWindow editorWindow = EditorWindow.GetWindow<SCPE_Window>(false, "About", true);
